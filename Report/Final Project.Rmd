...
## I. Introduction
This is a comprehensive Exploratory Data Analysis for billions of for-hire vehicle (Uber, Lyft, Via, etc) trips originating in New York City, and we focuse on trip counts and duration in New York competition with tidy R, ggplot2, and plotly.

The goal of this challenge is to process large data sets and to understand the duration of fhv in NYC based on features: trip location, pick-up, drop-off time, and weather effect. Also, we are interested in the difference betweeen three companies such as market shares, targeted customers, and business strategy. Firstly, we will study and visualize the original data, engineer new features, aggregate time-series variables. Then, we add external NYC weather data to analyze the impact on the trip duration. Finally, We compare three companies trips over various time frame on their target trip duration values.
...
```{r, echo=F}
setwd("E:/NYC Taxi Project/Analysis")
```
...
# df = fread("../Data/original/sample duration.csv")
...
Due to most companies allow to cancel the order in 2 minutes, and drives might miss the passengers in order to cancel the order. We find there are 279,693 records showing the trip duration is less than 2 minutes, and study on those pick-up and drop-off location by using **heat map**. Following heat map shows the number of trips which duration is less than 2 minutes. The pick-up and drop-off locations help to identify the possiblity.
...
cols = scales::col_numeric("Reds", domain = NULL)(vals)
colz = setNames(data.frame(vals[o], cols[o]), NULL)
plot_ly(data=df2, x=~pick_borough,y=~drop_borough,z=~counts, colorscale = colz, type = "heatmap",text = ~paste("Pick up: ",pick_borough, '<br>Drop off:', drop_borough,'<br>Trip counts:', n)) %>%
...
```{r, echo=F}
read.csv("./missing.csv") %>% as.tibble() %>% mutate(pick = mdy_hm(pick), drop = mdy_hm(drop))
```
...
p3 = read.csv("month duration.csv") %>%
...
* After mid-night, the shortest trip duration is falling down and reaching 16 minutes; the peak trip duration occurs during 6 AM - 6 PM about 20+ minutes in a typical day, and the longest trip duration is 24 minutest at 4 PM (intersting!)
...
### 5.2 types
We investigate on how the median trip duration dependes on the different companies in hourly, weekly and monthly bases.
```{r, fig.width= 8, fig.height= 4}
read.csv("hourly type duration.csv") %>% select(hour,type,d.med) %>%  accumulate_by(~ hour) %>%
...
read.csv("wday type duration.csv") %>%
  select(-X) %>%
  mutate(wday = ordered(wday,levels = lvl)) %>%
  accumulate_by(~ wday) %>%
  arrange(wday) %>%
...
```{r, fig.width= 8, fig.height= 4}
read.csv("month type duration.csv") %>%
  select(-X) %>%
...
```{r, fig.width= 8, fig.height= 4}
x = read.csv("./counts by h_wk_type.csv") %>%
  group_by(hour,type) %>%
  summarise(counts = sum(n))

y = read.csv("./counts by h_wk_type.csv") %>%
  group_by(hour) %>%
